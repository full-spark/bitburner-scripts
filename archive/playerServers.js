    ...
    while (true) {
      ...
      if (action == settings.actions.BUY) {
        ...
        if (smallestCurrentServer === biggestCurrentServer) {
          targetRam *= 4
          while (ns.getServerMoneyAvailable('home') * settings.totalMoneyAllocation >= ns.getPurchasedServerCost(4 * targetRam)) {
            targetRam *= 4
          }
        }
      ...
      if (!didChange) {
        ...
        ns.print(`Cost of server at target RAM: ${ns.formatNumber(ns.getPurchasedServerCost(targetRam), 3)}`)
